...
public class ModelClient : IDisposable
{
    ...
    // List of specific model names to remove
    private static readonly string[] s_modelsToRemove = [
        "Phi-4-multimodal-instruct", // does not work in Azure.
        "o1-preview", // o1-preview is older, so remove it in favor of o1.
        "gpt-4.5-preview", // gpt-4.5-preview is deprecated according to https://platform.openai.com/docs/models/compare
    ];
    ...
    private static void RunFixups(List<ModelEntity> allModels)
    {
        ...
        // Remove Phi-3 models since they are retired and not usable in Azure.
        allModels.RemoveAll(m => m.Annotations?.Name?.StartsWith("Phi-3") == true);

        allModels.RemoveAll(m => s_modelsToRemove.Contains(m.Annotations?.Name));
    }
}
